...
def create_spectrogram_images(x, sample_rate, win_size = 2, fs_fourier
= 128, overlap = 1):
...
def __create_spectrogram(x, win_size, fs_fourier, overlap):
    """
    Takes a one-dimensional time-series signal and converts it to a
time-frequency spectrogram image representation
    """
...
def __spectrogram(x=None, win=None, noverlap=None, nfft=None,fs=None,
sides='onesided'):
    """
    light weight approach to spectrogram calculation. basically a
slightly polished version of
    scipy.signal._fft_helper
    Syntax: times,  freqs, Sxx = spectrogram_lightweight(x=None,
win=None, noverlap=None, nfft=None,fs=None, sides='onesided')
    sides='onesided' or 'twosided'
    noverlap is in samples
    nfft is in samples (if  nfft is larger than win, then the signal
is zero padded)
    """
